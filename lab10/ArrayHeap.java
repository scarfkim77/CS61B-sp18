...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private static int parentIndex(int i) {
        /* TODO: Your code here! */
        return Math.max(i / 2, 1); //parent cannot be less than 1
    }
    ...
    private void swim(int index) {
        ...
        int min = min(index, parentIndex(index));
        if (min == index && min > 1) { //index.priority <= parent.priority; 如果index已经是root, 停止
    ...
    private void sink(int index) {
        ...
        /** TODO: Your code here. */
        int min = min(index, min(leftIndex(index), rightIndex(index)));
        if (min != index) { //index.priority >= min(left.index, right.index)
            swap(index, min);
            sink(min);
        }
        return;
